...
This is an experiment porting the [reRSS web
UI](https://framagit.org/ybon/rerss/blob/master/rerss/app/) (which is written
in [RIOT](https://riot.js.org/)) to elm.

To run:

* make sure you have `elm` and `create-elm-app` installed: `npm install elm create-elm-app`
* run the [server](https://framagit.org/ybon/rerss#install) locally
* start the elm web UI: `elm-app start`
